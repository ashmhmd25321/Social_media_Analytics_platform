...
import { dataCollectionService } from '../backend/src/services/DataCollectionService';
import { UserSocialAccount } from '../backend/src/models/SocialPlatform';
...
async function testMockDataCollection() {
  ...
  try {
    ...
    const account = accounts[0] as UserSocialAccount & { platform_name?: string };
    ...
    const [metrics] = await pool.execute<any[]>(
    ...
    const [jobs] = await pool.execute<any[]>(
    ...
    // Show sample posts
    const [samplePosts] = await pool.execute<any[]>(
